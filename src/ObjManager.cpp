	...
	FOREACH_OBJECT(o)
	{
		if (o != player && !o->deleted)		// player is moved in PDoPhysics
		{
			...
			else if (o->damage > 0)
			{
				...
				// SOLID_MUSHY objects doesn't let the player sink enough for a smaller hitbox to collide
...
// used by load_pxe to reset the game in preparation for loading a new maplayer->
