...
std::string ResourceManager::getLocalizedPath(const std::string& filename)
{
    ...
    if (home != NULL)
    {
        _tryPath=std::string(home)+"/.local/share/cavestory-nx/data/lang/"+std::string(settings->language)+"/"+filename;
        if (fileExists(_tryPath))
        {
            return _tryPath;
        }

        _tryPath=std::string(home)+"/.local/share/cavestory-nx/data/"+filename;
        if (fileExists(_tryPath))
        {
            return _tryPath;
        }
    ...
    _tryPath="/usr/share/games/cavestory-nx/data/lang/"+std::string(settings->language)+"/"+filename;
    if (fileExists(_tryPath))
    {
        return _tryPath;
    }

    _tryPath="/usr/share/games/cavestory-nx/data/"+filename;
    if (fileExists(_tryPath))
    {
        return _tryPath;
    }
    ...
    char *home = SDL_GetPrefPath("cavestory-nx", "nxengine");
    if (home != NULL)
    {
...
std::string ResourceManager::getPathForDir(const std::string& dir)
{
    ...
    if (home != NULL)
    {
        _tryPath=std::string(home)+"/.local/share/cavestory-nx/data/"+dir;
        if (fileExists(_tryPath))
        {
            return _tryPath;
        }
    ...
    _tryPath="/usr/share/games/cavestory-nx/data/"+dir;
    if (fileExists(_tryPath))
    {
        return _tryPath;
    }
